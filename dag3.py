...
"""
        ...
        # === NUEVO: COUNT(DISTINCT) por columna_id ===
        # Para cada fila (Database, Schema, Table, column_id) hacemos:
        # SELECT COUNT(DISTINCT [column_id]) FROM [db].[schema].[table]
        distinct_rows = []

        # Trabajamos sobre combinaciones únicas
        for _, r in (
...
"""
        ...
        # Opcional: XCom
        # ti.xcom_push(key="pk_with_rowcounts_and_distinct", value=df_final.to_dict(orient="records"))
...
# === Definición del DAG ===
with DAG(
    dag_id="bq_to_sqlserver_pk_catalog_pymssql_batches_cte_only_distinct_v1",
